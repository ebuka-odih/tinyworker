...
export class JobSearchRunStore {
  ...
  async upsertResult(runId: string, result: SearchRunResultItem): Promise<void> {
    const run = this.runs.get(runId)
    if (!run) return
  ...
  subscribe(runId: string, listener: (event: SearchRunEvent) => void): (() => void) | null {
    const run = this.runs.get(runId)
    if (!run) return null
  ...
  async getEventsSince(runId: string, sequence = 0): Promise<SearchRunEvent[]> {
    const run = this.runs.get(runId)
    if (run) {
  ...
  async getSnapshot(runId: string): Promise<Record<string, any> | null> {
    ...
    const persisted = await this.prisma.jobSearchRun.findUnique({
      where: { id: runId },
    })
